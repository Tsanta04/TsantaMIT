...
*/
		...
		//printf("<TABLE class='table table-borderless rounded' style='margin-bottom:8vh;'>\n");
			printf("<THEAD>\n");
				...
				printf("<TH>For user</TH>\n");
				printf("<TH>By user</TH>\n");
			printf("</THEAD>\n");
			printf("<TBODY>\n");
				for(int i=0;i<ligne;i++){
					...
					printf("<TD>%s</TD>\n",user[i].time);
					printf("<TD>%s</TD>\n",user[i].session);
					printf("<TD><a href='http://www.tsa.com/cgi-bin/adminLte/AdminLte/pages/examples/authLog.cgi?user=%s' style='text-decoration:none;'>%s</a></TD>\n",us,user[i].nameFor);
					printf("<TD>%s</TD></TR>\n",user[i].nameBy);
				}
