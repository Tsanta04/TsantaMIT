...
char* gamerRegister(int* num, int* score);
...
///Les variables
	...
	char* joueur;
	char langue[5][100]={"Famariparitana momba ny kilalao","Ireo kilalao misy","Fanampiana","Mari-dalao","Hiala"};
	char menu[5][100]={"Lyrics translating","Singer's name re-ordering","Guess the song","Hiverina any amin'ny pejy fandraisana","Ireo kilalao misy"};
		
///Initialisation
	resultat=(int*)malloc(sizeof(int)*3);
	joueur=(char*)malloc(100);
///tête

	//AffichageEntree();
	system("clear");
    affichage("Tongasoa ato amin'ny \033[34mVAZO MISOMA\033[0m!!!!");
    sleep(1);
    system("clear");

///ENregistrer le joueur
	strcpy(joueur,gamerRegister(&numero,resultat));
	//printf("%s",joueur);sleep(2);
...
///Traitement des choix
			...
			switch(n){		//Tous les choix
				case 0:		
						while(1){				//boucle pour revenir à la page d'acceuil ou non (X)
							...
							printf("\nTsindrio ny \033[33m< 1 >\033[0m raha hiverina ao amin'ny pejy fandraisana\n=>");
							reponse=0;
	...
	break;
				case 3: //scoreInfo(&resultat[0],&resultat[1],&resultat[2],&bestscore1,&bestscore2,&bestscore3);
						scoreInfo(bestscore1,bestscore2,bestscore3);
						clear();
	break;
			}
	break;			//Sort de la boucle pour l'entrée des numéros de choix d'acceuil, et revenir à la page d'acceuil (si on sort du boucle précédent)
...
///Les listes des artistes disponibles
	while(1){
	 ...
	 ici:
		printf("\t\t\t\033[36mVAZO MISOMA: Lyrics Translating\033[0m\n\n");
    	printf("\033[037mIty lalao ity dia mifototra amin'ny fandikana ampahan-tononkira nosoratra amin'ny teny Frantsay ho teny Malagasy.\n\033[0m");
       
        if(x==0){affichage("\nIreto avy ny Mpihira misokatra atreto:\n\n");}	

		sprintf(commande1,"cat /usr/local/fichier/listeArtistesExistants");
		system(commande1);
///Entrer dans le fichier de l'artiste
		printf("\nSafidio ny mpihira ankafizinao amin'ireo voalaza etsy eo ambony\033[33m(ampidiro ny anarana)\033[0m\n=>");
		scanf("%s",artistName);
		sprintf(commande1,"test -e /usr/local/fichier/%s/",artistName);
		test=system(commande1);
	//printf("%d",test);
		if(test!=0){
			printf("\033[5;31mMISY FAHADISOANA:Safidy tsy hita!!!\033[0m\n");
			sleep(2);
			system("clear");
			x++;
			goto ici;
			...
			sleep(1);
			system("clear");
			affichage("\n\t\tEny ary!!!! Miandry kely azafady indrindra\n");	
				   system("clear");    	
	            chargement(); 
			    sleep(2);
			    int u=0;
			while(1){
				...
				if(test==0){
					...
					sleep(2);
						...
						system("clear");
				///Debut du jeu;
				if(u==0){ affichage("Andao ary hilalao!!!!\nManana vintana \033[33m3\033[0m ianao haminavinana ny valiny marina.\n");
					      system("clear");
					      }
				here:
					...
					system("clear");
				test=123;
				char donnee2[3][200]={"Lyrics translating\n\n\tHiala anatin'ity lalao ity ve ianao sa mbola hanohy?","Quitter","Rejouer"};
...
void gameDescription(){
	//printf("\t\t\t\tVAZO MISOMA\n\n");
	printf("\t\t\t\033[36m Famariparitana momba ny kilalao\033[0m \n\n");
	system("cat ~/Mahaleo/descriptionJeu");
}
void revenirAcceuil(char *reponse){
	printf("Tsindrio ny ' x ' raha hiverina any amin'ny pejy fandraisana:\n");
	scanf("%c",reponse);
	//sleep(1);
		if(*reponse =='x'){
			system("clear");
...
///Indices
		if(i==1){
				printf("--> 'Sopapa' \033[33m(Ny lohatenin-kira)\033[0m:\n");
...
*/	
		...
		if(k!=j){ 
			//sleep(1);
			printf("\n~~Miala tsiny fa \033[31mdiso\033[0m!\n");
			if(i!=1){
				printf("(Vintana\033[33m%d\033[0m sisa.)\n\n",(i-1));
				sleep(2);
				system("clear");
				printf("(Vintana\033[33m%d\033[0m sisa.)\n\n",(i-1));
				//*resultat=0;
			...
			if(i==1){
				...
				printf("\t~~\033[31mVery\033[0m tamin'ity lalao ity ianao \033[33m%s\033[0m!!\n\n",joueur);
				printf("Ity no valiny marina: '%s'",tenatenavaliny);
			}
		}
		//if ((x)==0){
		if(k==j){
			...
			system("clear");
			printf("\t\t\t\033[36mVAZO MISOMA: Lyrics Translating\033[0m\n\n");
			printf("\n~~\033[32mArahabaina\033[0m!!\033[32mNaharesy\033[0mtamin'ity lalao ity ianao \033[33m%s\033[0m!!\n\n",joueur);
			(*resultat)++;
		break;
			}
	}
...
//	char f[10];
	...
	while(1){
		...
		char donnee1[3][200]={"Singer's name re-ordering\n\tIty lalao ity dia mifototra amin'ny fandaminana ny anarana mpihi-\n  ra Malagasy izay mikorontana.\tVONONA VE IANAO??","Vonona","Hiala"};
			if(x==0){
			...
			if(test==1){
				...
				clear();
				///Debut du jeu
			  affichage("Andao ary hilalao!!!!\nManana vintana \033[33m3\033[0m ianao haminavinana ny valiny marina.\n");
			   sleep(1);
			   system("clear");
			here:	debutJeu2(resultat,joueur);
					printf("\n");
					score(resultat,bestscore,joueur);
						//affichage("\tHiala anatin'ity lalao ity ve ianao sa mbola hanohy?\n\n");
				x++;
				test=123;
				char donnee2[3][200]={"Singer's name re-ordering\n\n\tHiala anatin'ity lalao ity ve ianao sa mbola hanohy?","Hiala","Hanohy"};
				test=request(donnee2);
				if(test==1){
					sleep(1);
					clear();
...
void score(int* resultat, int bestscore,char* joueur){	//les grades
	if(*resultat==bestscore){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m Nahazo ny \033[32m'Mari-dalao ambony indrindra'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	if((*resultat==5)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m\033[32m'ALOHA-DIAMONDRA'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==10)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0mVous êtes \033[32m'DIAMONDRA 1'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==20)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA 2'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==35)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA 3'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==50)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA 4'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==70)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA KINTANA 1'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==95)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA KINTANA 2'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==125)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0m \033[32m'DIAMONDRA KINTANA 3'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
	else if((*resultat==160)){
		system("clear");
		printf("\n\t\t\033[32m!!Arahabaina %s!!!\033[0mVous êtes \033[32m'DIAMONDRA KINTANA 4'\033[0m ianao izao!!\n",joueur);
		sleep(3);
		system("clear");
	}
}
...
///Indices
		if(i==1){
			printf("--> 'Sopapa' \033[33m(ny lohatenin-kirany)\033[0m:\n=>");
	  ...
	  for(int i=0;i<strlen(tenaizy);i++){
		...
		if(y!=0){ 
			...
			if(i==1){
				...
				printf("\t\t\t\033[36mVAZO MISOMA: Singer's name reordering\033[0m\n\n");
				printf("\t~~\033[31mVery\033[0m tamin'ity lalao ity ianao \033[33m%s\033[0m!!\n\n",joueur);
				printf("Ity no valiny marina: '%s'",valinkorontana);
			}
		}
		if (y==0){
			sleep(2);
...
///Annonce
	...
	attron(COLOR_PAIR(2));
	mvprintw(3,7, "Andao ary hilalao!!!!\nManana vintana 3 ianao haminavinana ny valiny marina.");
...
///Indices a la derniere tentative
	if(i==1){
		mvprintw(5,2,"--> Sopapa (lohatenin-kira)");
		sprintf(indice,"head -%d /usr/local/fichier/indiceKorontana|tail -1",m);
		indice=get(indice);
		mvprintw(5,30,"%s",indice);
		printf("\n");
	}
...
///Affichge de desordre
	WINDOW* window = newwin(14,55,7,7);
...
///Entrer la reponse
	...
	mvwprintw(reponse,1,1,"Ny valiny =>");
...
//	char f[10];
	...
	/*	printf("\t\t\t\033[36mVAZO MISOMA: Guess the Song\033[0m\n\n");
		...
		while(1){
		  ici:
		  printf("\t\t\t\033[36mVAZO MISOMA: Guess the Song\033[0m\n\n");
		  printf("  Ity lalao ity dia mofototra amin'ny faminavinana ny hira izay mandeha, amin'ny alalan'ny fanomezana ny lohateny sy ny mpihira ilay hira\n\n");
			 
			if(x==0){
			    affichage("\nIreto avy ary ireo karazan-kira misokatra atreto:\n\n");
		        sleep(2);
			 }
...
///Entrer dans le fichier de SONGS
			printf("Safidio ireo karazan-kira tianao hilalaovana \033[33m(ampidiro ilay anarany)\033[0m:\n=>");
			scanf("%s",categorie);
...
///Affichage
	...
	wattroff(win1,COLOR_PAIR(1));
		mvwprintw(win2,2,5,"Sorato tahaka izao ny valiny omenao:\t<Lohateny-Mpihira1&Mpihira2>");
		mvwprintw(win2,5,15,"FM:\tHenoy tsara ilay hira mandeha..");
		mvwprintw(win2,8,6,"Tsindrio ny <q> rehefa mijanona ny hira na rehefa vonona ");
		mvwprintw(win2,9,7,"ny hanome ny lohateny sy ny mpihira ilay hira ianao...");
	attroff(A_BOLD);
...
while(wgetch(stdscr)!='\n');
	...
	sleep(2);		
		...
		system("clear");
	    printf("\t\t\t\033[36mVAZO MISOMA: Guess the Song\033[0m\n\n");
		printf("\t\033[31mFM\033[0m\t: Henoy tsara ilay hira mandeha..\nTsindrio ny \033[33m<q>\033[0m rehefa mijanona ny hira na rehefa vonona ny hanome ny lohateny sy ny mpihira ilay hira ianao...\n\n");
		printf("\n\n\tAmpidiro ny \033[36mlohateny\033[0m sy ny \033[33mMpihira\033[0m (\033[36m'Lohateny'\033[0m: \033[33m'Anaran'ny mpihira'\033[0m):\n=>");
		fgets(valiny,100,stdin);
		if((i==3)&&(*repetition==0)){
		fgets(valiny,100,stdin);
		*repetition=1;
...
///Comparer
		...
		if(k!=j){ 
			...
			if(i==1){
				...
				printf("Toy izao ny valiny marina: '%s'",titreArtiste);
			}
		}
		if(k==j){
			sleep(2);
...
void aide(){
	...
	char f[10];
	int scan=0;
	char langue[5][100]={"Fanampiana ho an'ny 'lyrics Translating","Fanampiana ho an'ny 'Singer's name reordering'","Fanampiana ho an'ny 'Guess the song'","Hiala ato amin'ny 'Fanampiana'","Fanampiana"};
		...
		if(choix<=3 && choix>=0){
			if(choix==3){system("clear");break;}
				while(quitter!=1){
					switch(choix){
						case 0:system("clear");
						       printf("\t\t\t\033[36mFanampiana ho an'ny 'Lyrics Translating'\033[0m\n");
						       sprintf(AideLyrics,"cat ~/Mahaleo/AideLyrics");
							   system(AideLyrics);
					           printf("\nAmpidiro ny \033[33m<1>\033[0m raha hiala ato amin'ny Fanampiana ho an'ny 'Lyrics Translating'\n=> ");
	           ...
	           break;
				        ...
				        case 1:	system("clear");
				                printf("\t\t\t\033[36mFanampiana ho an'ny 'Singer's name reordering'\033[0m\n");
				                sprintf(AideReordering,"cat ~/Mahaleo/AideReordering");
								system(AideReordering);
								printf("\nAmpidiro ny \033[33m<1>\033[0m raha te hiala ato amin'ny Fanampiana ho an'ny 'Singer's name reordering'\n=> ");                         
                                scan=scanf("%d",&quitter);
					            if(scan==0){scanf("%s",f);}
                                system("clear");
               break;
					    ...
					    case 2: system("clear");
					     	    printf("\t\t\t\033[36mFanampiana ho an'ny 'Guess the song'\033[0m\n");
					     	    sprintf(AideSong,"cat ~/Mahaleo/AideSong");
					     	    system(AideSong);
		                        printf("\nAmpidiro ny \033[33m<1>\033[0m raha te hiala ato amin'ny Fanampiana ho an'ny 'Guess the song'\n=> ");
                                scan=scanf("%d",&quitter);
                                if(scan==0){scanf("%s",f);}
	                            system("clear");
               break;
...
void scoreRegister(int* resultat){	//soit on revient a 0 soit on continue les points
	int test=123;
	char donnee[3][200]={"Hanohy ny izay lalao farany ve ianao sa hanao lalao vaovao?","Hanohy","Lalao vaovao"};
	test=request(donnee);
	if(test==1){
	}
	if(test==0){
...
char* gamerRegister(int* num, int* score){	//faire entrer le joueur et enregistrer sont dernier point
///Les variables
	char tmp[100];
...
///Traitement d'affichage des inscrits
	...
	sprintf(tab[nombreJoueur],"Hanoratra anarana");
	...
	mvwprintw(stdscr,1,1,"Ireto avy ireo mpilalao efa voatahiry anarana atreto:");
/*
...
///Traitement de ce que le joueur entre
	if((fleche>=0)&&(fleche<nombreJoueur)){
		...
		joueur=tab[fleche];
		printf("\033[034mTongasoa i %s,%d,%d,%d!!\033[0m",joueur,score[0],score[1],score[2]);
	}
	else if(fleche==nombreJoueur){
		system("clear");
ici:	affichage("Ampidiro ny anaranao:\t");
		int id=0;
		scanf("%s",tmp);
		...
		for(int i=0;i<nombreJoueur;i++){
			...
			if(strcmp(tmp,joueurExistant)==0){
				id++;
			}
			//pclose(fp);
		}
		if(id>0){
			printf("\t\033[5;31mEfa voatahiry ato io anarana io.Azafady, mampidira aanarana hafa.\033[0m\n");
			sleep(2);
			system("clear");
			goto ici;
		}
		else if(id==0){
			sprintf(commande,"echo %s>>~/Documents/Projet/vazoMisoma/score/nom",tmp);
			system(commande);
			for(int i=0;i<3;i++){
		...
		joueur=tmp;
		printf("\033[34mTongasoa i %s!!\033[0m",joueur);
	}
	printf("\n");
	sleep(2);
	system("clear");
